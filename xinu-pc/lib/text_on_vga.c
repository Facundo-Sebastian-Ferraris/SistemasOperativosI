                         ...
                         unsigned char color)
{
        ...
        for (i = 0; i < 5; i++)
        {
                ...
                for (j = 0; j < 7; j++)
                {
                        if (check_bit(col, j))
                        {
                                pixel(x + i, y + j, COLOR_BLANCO);
                        }
                        else
                        {
                                pixel(x + i, y + j, COLOR_NEGRO);
                        }
                }
        }
}

void draw_char_on_buffer2(unsigned int x, unsigned int y, char c)
{
...
void print_text_on_vga2(unsigned int x, unsigned int y, char *text)
{
        int i = 0;
        const int offset = 6;
        char *c = text;
